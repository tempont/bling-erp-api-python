"""Contract tests for Bling Categorias - Produtos operations."""
...
from bling_erp_api.contracts.generated.product_categories import PRODUCT_CATEGORY_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_product_categories_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every CategoriasProdutos operation in OpenAPI."""
    ...
    for path, methods in paths.items():
        if path.startswith("/categorias/produtos"):
            for method, operation in cast("dict[str, object]", methods).items():
                ...
                if resource == "CategoriasProdutos":
                    spec_operations.add(
                        (
                            str(method).upper(),
                            path,
    ...
    contracted_operations = {
        ...
        for operation in PRODUCT_CATEGORY_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_product_categories_contracts_no_extra_operations() -> None:
    """Every PRODUCT_CATEGORY_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in PRODUCT_CATEGORY_OPERATIONS.values()}
