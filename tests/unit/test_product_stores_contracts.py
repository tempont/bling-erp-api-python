"""Tests for OpenAPI-derived Produtos lojas contracts."""
...
from bling_erp_api.contracts.generated.product_stores import PRODUCT_STORE_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_product_stores_contract_covers_openapi_operations() -> None:
    """Generated contracts should cover every ProdutosLojas operation."""
    ...
    official_operations = {
        ...
        if cast("dict[str, object]", operation).get("x-api-resource") == "ProdutosLojas"
    }

    contracted_operations = {
        (operation.method, operation.path, operation.action)
        for operation in PRODUCT_STORE_OPERATIONS.values()
    }
