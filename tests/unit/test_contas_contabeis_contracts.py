"""Contract tests for Bling Contas Financeiras operations."""
...
from bling_erp_api.contracts.generated.contas_contabeis import CONTAS_CONTABEIS_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_contas_contabeis_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every ContasContabeis operation in OpenAPI."""
    ...
    for path, methods in paths.items():
        if path.startswith("/contas-contabeis"):
    ...
    contracted_operations = {
        ...
        for operation in CONTAS_CONTABEIS_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_contas_contabeis_contracts_no_extra_operations() -> None:
    """Every CONTAS_CONTABEIS_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in CONTAS_CONTABEIS_OPERATIONS.values()}
