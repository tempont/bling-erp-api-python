"""Contract tests for Bling Homologacao operations."""
...
from bling_erp_api.contracts.generated.homologation import (
    HOMOLOGATION_OPERATIONS,
)

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_homologation_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every Homologacao operation in OpenAPI."""
    ...
    for path, methods in paths.items():
        if path.startswith("/homologacao"):
    ...
    contracted_operations = {
        ...
        for operation in HOMOLOGATION_OPERATIONS.values()
    }
    assert contracted_operations == spec_operations


def test_homologation_contracts_no_extra_operations() -> None:
    """Every HOMOLOGATION_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in HOMOLOGATION_OPERATIONS.values()}
