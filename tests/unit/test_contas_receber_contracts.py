"""Contract tests for Bling Contas a Receber operations."""
...
from bling_erp_api.contracts.generated.contas_receber import CONTAS_RECEBER_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_contas_receber_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every ContasReceber operation in OpenAPI."""
    ...
    for path, methods in paths.items():
        if path.startswith("/contas/receber"):
    ...
    contracted_operations = {
        ...
        for operation in CONTAS_RECEBER_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_contas_receber_contracts_no_extra_operations() -> None:
    """Every CONTAS_RECEBER_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in CONTAS_RECEBER_OPERATIONS.values()}
