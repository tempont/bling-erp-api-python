"""Contract tests for Bling Ad Categories (Anúncios - Categorias) operations."""
...
from bling_erp_api.contracts.generated.ad_categories import AD_CATEGORY_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_ad_category_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every AnunciosCategorias operation in OpenAPI."""
    ...
    for path, methods in paths.items():
        if path.startswith("/anuncios/categorias"):
            for method, operation in cast("dict[str, object]", methods).items():
                ...
                if resource == "AnunciosCategorias":
                    spec_operations.add(
                        (
                            str(method).upper(),
                            path,
    ...
    contracted_operations = {
        ...
        for operation in AD_CATEGORY_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_ad_category_contracts_no_extra_operations() -> None:
    """Every AD_CATEGORY_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in AD_CATEGORY_OPERATIONS.values()}
