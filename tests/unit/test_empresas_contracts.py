"""Contract tests for Bling Empresas operations."""
...
from bling_erp_api.contracts.generated.empresas import EMPRESA_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_empresas_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every Empresas operation in OpenAPI."""
    ...
    contracted_operations = {
        ...
        for operation in EMPRESA_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_empresas_contracts_no_extra_operations() -> None:
    """Every EMPRESA_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in EMPRESA_OPERATIONS.values()}
