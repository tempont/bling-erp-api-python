"""Tests for OpenAPI-derived Lotes (produtos) contracts."""
...
from bling_erp_api.contracts.generated.product_batches import PRODUCT_BATCH_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_product_batches_contract_covers_openapi_operations() -> None:
    """Generated contracts should cover every Lotes operation."""
    ...
    official_operations = {
        ...
        if cast("dict[str, object]", operation).get("x-api-resource") == "Lotes"
    }

    contracted_operations = {
        (operation.method, operation.path, operation.action)
        for operation in PRODUCT_BATCH_OPERATIONS.values()
    }
