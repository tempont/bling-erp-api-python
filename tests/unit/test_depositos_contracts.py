"""Contract tests for Bling Depósitos operations."""
...
from bling_erp_api.contracts.generated.depositos import DEPOSITO_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_depositos_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every Depositos operation in OpenAPI."""
    ...
    contracted_operations = {
        ...
        for operation in DEPOSITO_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_depositos_contracts_no_extra_operations() -> None:
    """Every DEPOSITO_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in DEPOSITO_OPERATIONS.values()}
