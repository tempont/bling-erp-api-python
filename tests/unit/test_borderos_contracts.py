"""Contract tests for Bling Borderôs operations."""
...
from bling_erp_api.contracts.generated.borderos import BORDERO_OPERATIONS

SPEC_PATH = Path("specs/bling-openapi-reference.json")


def test_borderos_contracts_cover_all_spec_operations() -> None:
    """Generated contracts should cover every Borderos operation in OpenAPI."""
    ...
    contracted_operations = {
        ...
        for operation in BORDERO_OPERATIONS.values()
    }

    assert contracted_operations == spec_operations


def test_borderos_contracts_no_extra_operations() -> None:
    """Every BORDERO_OPERATIONS entry action must map to a spec operation."""
    payload = cast("dict[str, object]", json.loads(SPEC_PATH.read_text(encoding="utf-8")))
    paths = cast("dict[str, object]", payload["paths"])
    ...
    contract_actions = {op.action for op in BORDERO_OPERATIONS.values()}
