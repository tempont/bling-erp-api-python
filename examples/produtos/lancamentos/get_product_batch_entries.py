...
def main() -> None:
    ...
    with BlingClient.from_env() as client:
        ...
        print("listar lancamentos:", lancs.model_dump_json(indent=2, by_alias=True))
        one = client.lotes_lancamentos.obter(id_lancamento)
        print("obter lancamento:", one.model_dump_json(indent=2, by_alias=True))
        soma = client.lotes_lancamentos.obter_saldos_soma(id_produto=id_produto)
        print("saldo soma (todos depositos):", soma.model_dump_json(indent=2, by_alias=True))
        soma_dep = client.lotes_lancamentos.obter_saldos_soma_deposito(
            id_produto=id_produto,
            id_deposito=id_deposito,
        )
        print("saldo soma deposito:", soma_dep.model_dump_json(indent=2, by_alias=True))
        saldos = client.lotes_lancamentos.obter_saldos(
            id_produto=id_produto,
            id_deposito=id_deposito,
            ids_lotes=[id_lote],
        )
        print("saldos por deposito:", saldos.model_dump_json(indent=2, by_alias=True))
        saldo = client.lotes_lancamentos.obter_saldos_saldo(
            id_produto=id_produto,
            id_lote=id_lote,
            id_deposito=id_deposito,
        )
        print("saldo lote+deposito:", saldo.model_dump_json(indent=2, by_alias=True))
